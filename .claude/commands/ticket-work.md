...
**Verify load-bearing claims in the ticket body — but only those.** The rule above governs *operational inputs* (env vars, Relevant Files paths, whether a prior script succeeded) — use them directly, never inspect them. It does **not** cover *technical assertions in the ticket body*: a claim that a specific field, DTO, endpoint, function, or behaviour exists in the code is a hypothesis about code state, not an input. When the implementation depends on such a claim, confirm it during the Phase 1 discovery reads you already perform — you are opening those files anyway, so this costs no extra turns. If a load-bearing claim proves false, **stop**: write `/tmp/jira_comment.txt` describing the gap and exit rather than building a workaround around the false premise. Scope this strictly to load-bearing claims — the specific symbols the change is built on — not every sentence in the ticket; do not turn discovery into a skeptical audit of the whole ticket. MS-87 shipped a fragile `figureId` workaround because a ticket note claimed a DTO field that did not exist, and nothing in the run ever opened that DTO to check.
...
2. **Phase 1 – Discovery (one batch turn).** Call `Read` on every file you need, all at once. Do not start implementing until this batch is complete. Any file you will edit must be in this batch — including docs like `CLAUDE.md`, module `CLAUDE.md` files, and skill files — so that no later `Edit` hits the "file has not been read yet" guard.
   ...
   - UI change to an *existing* screen → in this same batch, `grep` **once** for a `captureRoboImage` block covering that screen (e.g. grep the screen name across `composeApp/src/androidUnitTest`). Record whether one already exists — step 5 uses this to decide whether to author a new render block, so it never needs a separate lookup there. Do not open the render test to read it end-to-end; the grep hit (or miss) is the only signal step 5 needs.
...
5. **If the change touches any `composeApp` composable, render it and self-critique before shipping.** You cannot see the UI otherwise, so this is the only proofing step before a human pulls the branch.
   - **Ensure a render block exists for each affected screen, then build once.** Using the render-block existence check you already did in discovery (step 2), split the affected screens into two cases — do **not** re-inspect the test files here:
