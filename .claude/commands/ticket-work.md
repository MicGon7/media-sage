...
1. The ticket is already In Progress — do not transition it again. Fetch the ticket and set up the branch in one call:
   ...
   ```
   The fetch script prints the full ticket text directly in this tool result — summary, Context, Acceptance Criteria, and Relevant Files (if the ticket has one) — that is your input to Phase 1 discovery in step 2. Use what's printed here; do not `echo` a `$TICKET_*` var or `cat` `/tmp/worker_ticket.env` afterward to re-view it — exported vars only live in the shell process that sourced them, so a later, separate `Bash` call can't see them anyway, and everything you need was already printed once. The init script derives the branch slug from `$TICKET_SUMMARY` automatically, and precomputes the render-test coverage list (`$WORKER_RENDER_COVERED` — the composables that already have a `captureRoboImage` block). Use that list in steps 2 and 5 instead of grepping for render blocks yourself.
