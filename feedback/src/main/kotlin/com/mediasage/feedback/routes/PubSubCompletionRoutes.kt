package com.mediasage.feedback.routes
 ...
 * table (written by the orchestrator via `:pipeline-core`) and records the outcome. It never
 * mutates the row — the orchestrator owns job-state transitions. Because the Analyst is never in
 * the critical path, the handler simply acknowledges with 200; Pub/Sub retries on any non-2xx, so
 * a cold start or transient blip is harmless.
 *
 ...
 */
fun Route.pubSubCompletionRoutes(webhookSecret: String, jobRegistry: JobRegistry) {
    post("/webhook/pubsub") {
