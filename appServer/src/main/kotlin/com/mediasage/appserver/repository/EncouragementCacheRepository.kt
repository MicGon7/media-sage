...
class EncouragementCacheRepository {
    ...
    suspend fun getByArticleUrlAndLocale(articleUrl: String, locale: String): EncourageResult? =
        withContext(Dispatchers.IO) {
            transaction {
                EncouragementCacheTable.selectAll()
                    .where {
                        (EncouragementCacheTable.articleUrl eq articleUrl) and
                            (EncouragementCacheTable.locale eq locale)
                    }
                    .singleOrNull()
                    ?.toEncourageResult()
            }
    ...
    suspend fun insert(articleUrl: String, locale: String, result: EncourageResult, cachedAt: Long) =
        withContext(Dispatchers.IO) {
            transaction {
                EncouragementCacheTable.insertIgnore {
                    it[EncouragementCacheTable.articleUrl] = articleUrl
                    it[EncouragementCacheTable.locale] = locale
