...
- **Leave no dead code — remove what your change orphans:** When a change makes code unreachable (a now-unused function, class, intent, field, or a whole vertical), delete it in the *same* PR — do not leave it "in case we go back to it later." Git history is the way back; a lingering orphan is pure carrying cost (every `grep` hit, every refactor, every Fake forced to implement an unused method). This is not an observable outcome, so it will not appear in the AC — do it anyway. Scope it strictly to what *your* change orphaned, and `grep` the symbol to confirm zero remaining references before deleting; do not hunt pre-existing dead code across the codebase. **Guardrail — destructive schema/data changes are a decision, not a cleanup:** dropping a Room table/entity, or a migration that discards persisted user data, is potentially irreversible. Do **not** auto-apply it — surface it in the PR body and Jira comment as an explicit decision for the human (the same escalation as a boundary-crossing load-bearing claim in `ticket-work.md`), and let them decide.
