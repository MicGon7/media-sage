...
### `AppViewModel` Wiring
The existing offline-first `seedDefaultsIfEmpty()` call in `init` is untouched. A second, independent coroutine reacts to `authState` becoming `Authenticated` with a non-blank `userId` (excluding the `bypassAuth()` path) and calls `syncWithRemote(userId)`, `distinctUntilChanged()` so it only fires once per signed-in account per app run.
