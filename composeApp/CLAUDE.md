...
- **Every new screen or reusable UI component ships with a `@Preview`**: New screen composables and new reusable UI components (cards, carousels, list items, etc.) must include at least one `@Preview` composable in the same file, using realistic sample data — this catches layout, alignment, and color issues in the IDE instead of a full build-deploy-navigate round trip. Follow `BriefingScreen.kt`'s `BriefingScreenPreview`: a single `@Preview(showBackground = true)` composable that wraps the screen in `MediaSageTheme` and passes realistic sample data directly into the state constructor. Add more than one preview only when a state materially changes layout (e.g. light/dark, populated/empty).
