...
**`worker-ship.sh` prints its own result block.** Do not add a text response after it succeeds — that would violate CLAUDE.md's "no narration between steps" rule.
...
1. The ticket is already In Progress — do not transition it again. Fetch the ticket and set up the branch in one call:
   ...
   ```
   The fetch script prints the full ticket text directly in this tool result — summary, Context, Acceptance Criteria, and Relevant Files (if the ticket has one) — that is your input to Phase 1 discovery in step 2. Use what's printed here; do not `echo` a `$TICKET_*` var or `cat` `/tmp/worker_ticket.env` afterward to re-view it (per CLAUDE.md's "trust your operational inputs" rule — everything you need was already printed once). The init script derives the branch slug from `$TICKET_SUMMARY` automatically, and precomputes the render-test coverage list (`$WORKER_RENDER_COVERED` — the composables that already have a `captureRoboImage` block). Use that list in steps 2 and 5 instead of grepping for render blocks yourself.
...
2. **Phase 1 – Discovery (batched, at most 2–3 rounds).** The ticket gives you the goal (Context + AC) and normally a `Relevant Files` hint. Discovery is always **batched**: each round fires every `Read` you can name in one parallel message, in **at most 2–3 rounds**. Round 1: `Read` the hinted files directly (they are your candidate set — do not `grep` to pre-confirm them); if there is no hint, run your locating searches together first, then batch-`Read` every file the results let you name. A round 2 (and, at most, a round 3) exists *only* to read files the previous round's imports or composition just *revealed*, files the hint missed, or a path a failed `Read` forces you to relocate — e.g. you read `ReaderScreen`, learn it composes `OverridePickerSheet` and `ReporterScheduleSection`, and batch-read those together next. Never open a round to read a single file you could have named earlier. Do not start implementing until your reads return. Any file you will edit must be read first — including docs like `CLAUDE.md`, module `CLAUDE.md` files, and skill files — so that no later `Edit` hits the "file has not been read yet" guard.
   ...
   **To fill gaps the hint missed — or when the ticket carries no hint — build the candidate set yourself.** Infer where the code lives from the package layout documented in `CLAUDE.md`, then confirm by locating it — up front, in as few search round-trips as possible, before you read. Run the locating searches you need together, not one-then-decide-the-next: `grep` for the symbol, feature name, or string the change hinges on; `Glob` for a file whose exact path you cannot infer. **Never delegate file location to a sub-agent (Explore, Task, or any other) — do all discovery directly in this main loop.** A sub-agent runs in a separate context: you must still `Read` every file you edit here (the Edit guard requires it), so a sub-agent only re-reads files you then read again — duplicated cost with no saving. Direct `grep`/`Glob` + a batched `Read` locates any well-scoped ticket's files in this documented codebase, up to the largest feature-sized ones — **a large or cross-cutting ticket scope is never itself grounds to delegate discovery to a sub-agent.** The line that matters is the *kind* of question, not the ticket's size:
   - **Exhaustive-enumeration discovery — never delegate, regardless of scope.** "Find every implementer of interface X" (every Fake, every DAO consumer, every subtype of a sealed class) has one correct answer a direct, exhaustive `grep` guarantees. A sub-agent's report is a manual, non-exhaustive survey — it can silently miss implementers, and you will not know it did. MS-647 dispatched a sub-agent for exactly this ("find any existing Fakes that implement DailyReflectionRepository"); it found one of three, the omission wasn't visible in its report, and the first build failed on the two Fakes it never surfaced.
   - **Genuinely open-ended discovery — the only case a sub-agent may help.** A question the ticket scope truly cannot answer up front, with no enumerable correct-answer set to miss (e.g. "how does this codebase typically structure X" as background research, not "which files must I edit"). Even here, a sub-agent only *informs* your read plan — you still `Read` every file you go on to edit yourself.

   Beyond the file the change obviously targets, pull in the co-located files a change silently fans out to — these are the easy-to-miss edit targets, not derivable from a single path:
...
5. **If the change alters a composable's rendered output, render it and self-critique before shipping — scoped to the screen(s) you touched.** You cannot see the UI otherwise, so this is the only proofing step before a human pulls the branch. If `WORKER_SKIP_UI_RENDER` is `true` in your environment, skip this entire step (an operator-set emergency override, not the default) and proceed straight to step 6. The trigger is a change to what the composable *draws in its default snapshot* — layout, text, visibility, colour, or an on-screen element added or removed in the sample-state render. A change that only affects **behaviour or interaction** — a click handler, a tap-triggered sheet or dialog, navigation, or data filtering not reflected in the sample state — produces an *identical* Roborazzi snapshot, so it gets **no** render test and skips this step entirely. Roborazzi captures one static snapshot with sample state; it cannot exercise interaction, so authoring a render test for a behaviour-only change proves nothing and wastes a build. Editing a `composeApp` file is not itself the trigger — ask whether the default snapshot changes.

   - **Check the remaining time budget before doing anything else in this step.** The render step is sequenced last, after discovery, implementation, tests, and detekt — so if the run has already run long, render is the step most likely to still be in flight when the Cloud Run job's timeout hits, and a timeout here loses the entire run's completed work (nothing is committed or pushed until step 7). Run:
     ```bash
     REMAINING=$(( ${JOB_TIMEOUT_SECONDS:-2700} - ($(date +%s) - ${JOB_START_EPOCH:-$(date +%s)}) ))
     echo "Time remaining in job budget: ${REMAINING}s"
     ```
     If `REMAINING` is under 480 (8 minutes) — enough for a render, one fix-and-re-render retry, and some margin for a slow/cold container — skip the rest of this step entirely (do not author a render block, do not run `capture-ui.sh`) and go straight to step 6/7 to ship the completed implementation. Note in the PR body and Jira comment that the render step was skipped for running low on time, distinct from the no-SDK/`WORKER_SKIP_UI_RENDER` skip reasons capture-ui.sh already reports. Shipping the completed work without screenshots is always better than losing it to a timeout mid-render.
   ...
   - **Scope the render to only the screen(s) you touched.** `recordRoborazziDebug` renders every `captureRoboImage` block in the whole app by default — with 15+ accumulated across the codebase, an unscoped render wastes most of the step's time re-rendering screens this ticket never touched. Pass the fully-qualified render-test class name(s) for the affected screen(s) as arguments: `./scripts/capture-ui.sh com.mediasage.feature.foo.FooScreenRenderTest`. Never call `capture-ui.sh` with no arguments on a real ticket — that renders everything.
   - Run `./scripts/capture-ui.sh <TestClass...>` **exactly once, in the foreground** — a single blocking `Bash` call. Do not launch it as a background task and then poll `TaskOutput`; the script prints its `## UI screenshots` block and exits on its own, so a background run only adds redundant poll turns (the MS-87 run wasted a turn on a non-blocking poll immediately followed by a blocking one). Never run it before the `captureRoboImage` block is written: running it against a missing test both wastes a build and exits 1 ("render produced no PNGs"), forcing a second build after you write the test. It stages the PNGs and prints a `## UI screenshots` Markdown block. If it exits 3 with a "no Android SDK" notice (or `WORKER_SKIP_UI_RENDER=true`), the render is *skipped, not failed* — continue the run without screenshots.
   - **`Read` the full PNG directly — never write or run a script to decode, parse, or crop it.** Claude's vision handles the whole image; no image tooling is required to critique padding, alignment, spacing, truncation, theme colours, or a missing-image fallback. If you genuinely need a closer look at one region (rare), the worker image ships ImageMagick — use `convert screenshot.png -crop WxH+X+Y cropped.png` and `Read` the crop. Do not hand-roll a PNG parser under any circumstance; that has previously looped without ever converging and blown the job's timeout.
   - **At most one fix-and-re-render cycle.** If the screenshot reveals a real issue, fix the composable and re-run the script once more. Do not re-render a third time — if something still looks off after one fix, ship anyway and note the remaining concern in the PR body rather than continuing to loop.
