...
object FigureSeeder {
    ...
    private val figures = listOf(
        ...
        FigureSeed("Jonathan Edwards", "theologian", "18th", "Theologian & Revivalist", "1703-1758",
            wikipediaTitle = "Jonathan Edwards (theologian)"),
        ...
        FigureSeed("William Carey", "theologian", "18th", "Missionary & Translator", "1761-1834",
            wikipediaTitle = "William Carey (missionary)"),
        ...
        FigureSeed("Andrew Murray", "missionary", "19th", "Pastor & Author", "1828-1917", wikipediaTitle = "Andrew Murray (minister)")
    )
    ...
    suspend fun seed(httpClient: HttpClient) = withContext(Dispatchers.IO) {
        ...
        val emptyBioCount = transaction {
            FigureTable.selectAll().where { FigureTable.bio eq "" }.count()
        }
        if (existingCount >= figures.size && emptyBioCount == 0L) {
            logger.info("Figures already seeded ($existingCount records, no empty bios) — skipping")
            return@withContext
        }
        if (existingCount >= figures.size && emptyBioCount > 0L) {
            logger.info("Re-fetching bios for $emptyBioCount figures with missing bio...")
        }

        logger.info("Seeding ${figures.size} figures...")
        figures.forEachIndexed { index, figure ->
            val bio = fetchWikipediaBio(httpClient, figure.wikipediaTitle ?: figure.name)
            upsertFigure(figure, bio)
            logger.info("Seeded (${index + 1}/${figures.size}): ${figure.name}")
            if (index < figures.size - 1) delay(500)
        }
        logger.info("Figure seeding complete")
    }

    private fun upsertFigure(figure: FigureSeed, bio: String) = transaction {
    ...
    private suspend fun fetchWikipediaBio(httpClient: HttpClient, name: String): String {
        return try {
            ...
            val response = httpClient.get("https://en.wikipedia.org/api/rest_v1/page/summary/$encodedName") {
                header("User-Agent", "MediaSage/1.0 (https://github.com/MicGon7/media-sage)")
            }
            val json = Json.parseToJsonElement(response.bodyAsText()).jsonObject
            val extract = json["extract"]?.jsonPrimitive?.content?.trim() ?: ""
            if (extract.contains("may refer to:") || json["type"]?.jsonPrimitive?.content == "disambiguation") {
                logger.warn("Wikipedia returned disambiguation page for $name — add a wikipediaTitle override")
                ""
            } else {
                extract
                    .replace(" ", " ")
